...
public class RoleController {
	 ...
	 * @PostMapping("/newRole")
	 * 
	 * @Operation(summary = "Create Role", description =
	 * "Creates a new role in the system") public JsonResponseEntity<Role>
	 * createNewRole(@RequestBody final RoleRequestDto role) throws
	 ...
	 * response.setStatus(StringConstants.success); response.setResult(newRole);
	 * response.setException(null);
	 * response.setMessage(StringConstants.roleCreatedMessage);
	 * response.setStatusCode(HttpStatus.CREATED); } return response; }
	 */
	 ...
	 * "Deletes a role based on the provided role ID") public
	 * JsonResponseEntity<Role> deleteRole(@PathVariable final int roleId) throws
	 * ResourceNotFoundException { final Role role = roleService.deleteRole(roleId);
	 * 
	 * JsonResponseEntity<Role> response = new JsonResponseEntity<>();
	 * 
	 * if (CommonUtils.isValidObject(role)) {
	 * response.setStatus(StringConstants.success); response.setResult(role);
	 * response.setException(null);
	 * response.setMessage(StringConstants.RoleDeletedMessage);
	 * response.setStatusCode(HttpStatus.OK); } else {
	 * response.setStatus(StringConstants.failed); response.setResult(null);
	 * response.setException(null);
